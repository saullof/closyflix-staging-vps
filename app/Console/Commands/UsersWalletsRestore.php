    ...
    public function handle()
    {
        ...
        foreach($users as $user){
            ...
            foreach($moneyReceivedTransactions as $transaction){
                $moneyReceivedNoTaxes += (float)$transaction->amount - (float)data_get($transaction->decoded_taxes, 'taxesTotalAmount', 0);
            }
            foreach($usersToAdjust as $k => $userToAdjust){
                if((int)$userToAdjust->user_id === (int)$user->id){
