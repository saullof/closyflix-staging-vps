            ...
            try{
                ...
                if ($subscription->provider === Transaction::CREDIT_PROVIDER) {
                    ...
                    if ($renewalTransaction != null && $renewalTransaction->status === Transaction::APPROVED_STATUS) {
                        ...
                        $subscription->expires_at = Carbon::instance(new \DateTime('+'.PaymentsServiceProvider::getSubscriptionMonthlyIntervalByTransactionType($subscription->type).' month', new \DateTimeZone('UTC')));
                    }
                    $subscription->save();
                } else {
