<nav class="sidebar {{(Cookie::get('app_theme') == null ? (getSetting('site.default_user_theme') == 'dark' ? '' : 'light') : (Cookie::get('app_theme') == 'dark' ? '' : 'light'))}}">
    ...
    <ul class="list-unstyled menu-elements p-0">
        ...
        @if(GenericHelper::isEmailEnforcedAndValidated())
            ...
            <li class="{{Route::currentRouteName() == 'my.coupons.index' ? 'active' : ''}}">
                <a class="scroll-link d-flex align-items-center" href="{{route('my.coupons.index')}}">
                    @include('elements.icon',['icon'=>'ticket-outline','variant'=>'medium','centered'=>false,'classes'=>'mr-2'])
                    {{__('Coupons')}}</a>
            </li>
