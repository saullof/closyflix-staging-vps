    ...
    <div class="flex gap-3 pr-4">
        ...
        <div class="d-flex v-align-center mb-1">
            ...
            <p class="font-semibold text-yellow-800 dark:text-yellow-300 ml-1 mb-0">
                {{ __('Spotify requires a Callback URL.') }}
            </p>
        </div>

        <div class="space-y-2">
            <p class="text-sm mb-1">
                {{ __('Add the following Redirect URI in your Spotify Developer Dashboard:') }}
            </p>

            <ul class="list-disc list-inside text-sm mb-0">
                <li>
                    <code>{{ route('my.settings.spotify.callback') }}</code>
                </li>
            </ul>

            <p class="text-sm mt-2 mb-0">
                {{ __('Make sure this matches your current environment domain.') }}
            </p>
        </div>
    </div>
