<form action="{{ route('search.get')}}" class="search-box-wrapper w-100 {{isset($formClasses) ? $formClasses : ''}}" method="GET">
    <div class="input-group input-group-seamless-append">
        <input type="text" class="form-control shadow-none {{isset($inputClasses) ? $inputClasses : ''}}" aria-label="Text input with dropdown button" placeholder="{{__("Search")}}" name="query" value="{{isset($searchTerm) && $searchTerm ? $searchTerm : ''}}">
