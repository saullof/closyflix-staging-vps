...
var UsersPaginator = {
    ...
    loadResults: function (direction='next') {
        ...
        if(UsersPaginator.isFetching === true || !url){
            return false;
        }
            ...
            success: function(result) {
                if(result.success){
                    ...
                    if(direction !== 'prev' && result.data.hasMore !== false){
                        UsersPaginator.scheduleViewportFill();
                    }
                }
                else{
    ...
    scheduleViewportFill: function(){
        window.clearTimeout(UsersPaginator.fillTimer);
        UsersPaginator.fillTimer = window.setTimeout(function () {
            UsersPaginator.fillViewport();
        }, 80);
    },
